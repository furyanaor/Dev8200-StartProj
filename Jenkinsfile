  ...
  stages {
    ...
    stage('DeployAppOnTesting') {
      steps {
        ...
        sh "sudo ssh -i /home/ec2-user/.ssh/id_dsa ec2-user@52.90.177.233 'docker-compose -f ~/testingfile/DC4Servers/docker-compose.yml down'"
        
        // clon docker-compose&testing file from github (its just clone all the repo)
        //sh "sudo ssh -i /home/ec2-user/.ssh/id_dsa ec2-user@52.90.177.233 'if sudo ls -lart ~/testingfile; then sudo ls -lart ~/testingfile; fi'"
        sh "sudo ssh -i /home/ec2-user/.ssh/id_dsa ec2-user@52.90.177.233 'sudo rm -rf ~/testingfile'"
        //sh "sudo ssh -i /home/ec2-user/.ssh/id_dsa ec2-user@52.90.177.233 'sudo ls -lart ~/'"
        sh "sudo ssh -i /home/ec2-user/.ssh/id_dsa ec2-user@52.90.177.233 'sudo git clone https://github.com/furyanaor/Dev8200-StartProj.git ~/testingfile'"
        sh "sudo ssh -i /home/ec2-user/.ssh/id_dsa ec2-user@52.90.177.233 'sudo ls -lart ~/testingfile'"

        // Clear old docker images
        sh "sudo ssh -i /home/ec2-user/.ssh/id_dsa ec2-user@52.90.177.233 'ls -la /home'"
        sh "sudo ssh -i /home/ec2-user/.ssh/id_dsa ec2-user@52.90.177.233 'if sudo docker ps | grep dev8200-startproj_web.name.latest; then sudo docker stop dev8200-startproj_web.name.latest; fi'"
        sh "sudo ssh -i /home/ec2-user/.ssh/id_dsa ec2-user@52.90.177.233 'if sudo docker images | grep dev8200; then sudo docker image rm -f furyanaor/dev8200-startproj_web:latest; fi'"
        // sh "sudo ssh -i /home/ec2-user/.ssh/id_dsa ec2-user@52.90.177.233 'sudo docker run --rm -d -p 7007:80 --name dev8200-startproj_web.name.latest furyanaor/dev8200-startproj_web:latest'"
      
        // docker-compose up on Production Virtual Server
        sh "sudo ssh -i /home/ec2-user/.ssh/id_dsa ec2-user@52.90.177.233 'docker-compose -f ~/testingfile/DC4Servers/docker-compose.yml up --build -d'"
      }
    }
    ...
    stage('TestingWeb') {
      steps {  
        ...
        sh "sudo ssh -i /home/ec2-user/.ssh/id_dsa ec2-user@52.90.177.233 'sudo bash ~/testingfile/DC4Servers/testmydocker.sh'"
        //input(id: "Deploy Gate", message: "Deploy ${params.project_name}?", ok: 'Deploy')
      }
    }
